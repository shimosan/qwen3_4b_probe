...
3. **top1 は dtype に関わらず常に一致**（fp16 でも fp32 でも 37/37 match）。これは「top1 と次点の logit 差が量子化誤差（±0.06 程度）より大きい層では順位が保たれる」ことを意味し、本実験では全 37 層で top1 が一致した。一般には差が量子化誤差に埋もれる層で順位が入れ替わりうる点に注意。実用上、講義デモは fp16 で十分。
...
- **講義デモ・nb02 では fp16 / MPS で十分**: top1 match が保証されているので、可視化や ranking 表示は fp16 の量子化誤差に影響されない
